# file: roop/processors/frame/face_swapper_hybrid.py
...
try:
    from roop.processors.frame.face_enhancer import get_face_enhancer  # enhancer-final integration
except Exception:
    get_face_enhancer = None

# ===================== OneEuroFilter (dari versi lama) =====================
class OneEuroFilter:
...
ONE_EURO_CONFIG = getattr(roop.globals, 'one_euro_config', {
    'freq': 30.0, 'min_cutoff': 1.0, 'beta': 0.007, 'd_cutoff': 1.0
})
...
def pre_check() -> bool:
    ...
    conditional_download(download_directory_path, [
        # maintain compatibility with both urls from original files
...
def pre_start() -> bool:
    # ensure source is image and has face
    ...
    if not get_one_face(cv2.imread(roop.globals.source_path)):
        ...
        return False
    # ensure target is image or video
...
# ---------- pose-aware bbox adaptation (dari file instruksi) ----------
...
# ---------- robust alignment menggunakan landmarks (dari versi lama) ----------
def robust_face_alignment(source_face: Face, target_face: Face, temp_frame: Frame) -> Tuple[Frame, np.ndarray]:
    try:
        ...
        if sm_source is None or sm_target is None or len(sm_source) < 3 or len(sm_target) < 3:
            return temp_frame, np.eye(2, 3, dtype=np.float32)
...
# ---------- color correction & blending (dari versi lama) ----------
...
def seamless_face_blending(swapped_face: Frame, target_frame: Frame, target_face: Face) -> Frame:
    try:
        ...
        try:
            ...
            if swapped_face.shape[:2] != face_region.shape[:2]:
            ...
            target_frame[y1:y2, x1:x2] = blended
            return target_frame
        except Exception:
            return target_frame
...
def swap_face_hybrid(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    try:
        ...
        # fallback to simple paste_back swap to avoid crash
        try:
    ...
    # 5) blending back to frame
    result = seamless_face_blending(swapped_frame, temp_frame, target_face)
    # 6) optional post-enhancer (GFPGAN) blending if available and enabled
    try:
        if get_face_enhancer is not None and getattr(roop.globals, 'face_enhancer_blend', None) is not None:
            # If enhancer present, apply minimal pathway: enhance crop & blend using enhancer settings
            enhancer = get_face_enhancer()
            # extract bbox region again after blending (safe)
            x1, y1, x2, y2 = map(int, target_face.bbox)
            h, w = result.shape[:2]
            x1, y1 = max(0, x1), max(0, y1)
            x2, y2 = min(w, x2), min(h, y2)
            ...
            if crop.size:
                with threading.Semaphore():
                    try:
                        ...
                        # blend with fidelity = face_enhancer_blend (0..1)
                        fidelity = float(getattr(roop.globals, 'face_enhancer_blend', 0.6))
...
# ---------- frame processor glue (mempertahankan many-faces flag, embedding matching, occlusion) ----------
def process_frame(source_face: Face, reference_face: Face, temp_frame: Frame, frame_number: int = 0) -> Frame:
    ...
    if getattr(roop.globals, 'many_faces', False):
        ...
        for idx, f in enumerate(faces):
            ...
            if detect_occlusion(f, temp_frame):
                continue
            temp_frame = swap_face_hybrid(source_face, f, temp_frame)
        return temp_frame
    # single face mode -> try tracking + embedding selection
    ...
    if best is None:
        best = valid[0]
    temp_frame = swap_face_hybrid(source_face, best, temp_frame)
    return temp_frame
...
def process_video(source_path: str, temp_frame_paths: List[str]) -> None:
    # set reference_face for single-face mode
