...
# Integrasi dengan Face Analyser baru
...
def get_face_enhancer() -> Any:
    ...
    with THREAD_LOCK:
        if FACE_ENHANCER is None:
            ...
            if 'CoreMLExecutionProvider' in roop.globals.execution_providers: device = 'mps'
...
def apply_blend_and_color_match(enhanced_crop: np.ndarray, original_crop: np.ndarray, fidelity: float) -> np.ndarray:
    try:
        ...
        corrected_crop = np.clip(enhanced_crop.astype(np.float32) + color_diff, 0, 255).astype(np.uint8)

        # Fidelity Blending
        blended = cv2.addWeighted(corrected_crop, fidelity, original_crop, 1.0 - fidelity, 0)

        # Masking (Occlusion & Box Removal)
        ...
        return (blended * mask_3ch + original_crop * (1.0 - mask_3ch)).astype(np.uint8)
    except:
...
def process_frame(source_face: Face, reference_face: Face, temp_frame: Frame) -> Frame:
    ...
    if many_faces:
        for target_face in many_faces:
            # 📌 SAFETY CHECK: Jangan enhance kalau wajah terdeteksi oklusi (misal tertutup tangan)
